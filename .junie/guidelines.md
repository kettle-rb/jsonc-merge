Project: jsonc-merge — Development Guidelines (for advanced contributors)
...
- Gem signing (for releases)
  ...
  - Important for local testing (to avoid hanging prompts): ALWAYS skip signing when building locally to test the packaging or install process. Without the private key password, the build will wait indefinitely at a signing prompt.
    - One-off commands (recommended):
      - SKIP_GEM_SIGNING=true gem build jsonc-merge.gemspec
...
- Running tests (verified)
  ...
  - Focused runs
    ...
    - Example: K_SOUP_COV_MIN_HARD=false bin/rspec spec/jsonc-merge/class_spec.rb:42
  - Output visibility
...
- Clarification: Executable scripts provided by this gem (exe/* and installed binstubs) work when the gem is installed as a system gem (gem install jsonc-merge). However, the Rake tasks provided by this gem require jsonc-merge to be declared as a development dependency in the host project's Gemfile and loaded in the project's Rakefile.
